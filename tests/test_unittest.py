        ...
        for line in runner.stdout.decode("utf-8").split("\n"):
            try:
                ...
                if "path" in payload:
                    ...
                    self.fail(f"Unknown payload line: '{payload}'")
...
class SuiteJoinTests(unittest.TestCase):
    ...
    def test_join_case(self):
        ...
        self.assertEqual(
            suite.join_path(parent, TestCase, "TestClass"), "tests.module.TestClass"
        )

    def test_join_module(self):
        suite = UnittestTestSuite()
        parent = TestModule(None, "tests", "tests")
        self.assertEqual(suite.join_path(parent, TestModule, "module"), "tests.module")

    def test_join_submodule(self):
        suite = UnittestTestSuite()
        self.assertEqual(suite.join_path(suite, TestModule, "tests"), "tests")
